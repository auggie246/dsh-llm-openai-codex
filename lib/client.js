...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      card: { width: '100%', maxWidth: 720, padding: 18, border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 10, background: 'var(--dsw-alias-bg-layer-3)', color: 'var(--dsw-alias-label-primary)', display: 'grid', gap: 14 },
    ...
    async function apply(ctx) {
      ...
      ctx.slots.inject('settings.plugin.item', () => ctx.slots.register({
        ...
        inject: () => ({ remote: ctx.remote.codexAuth }),
      }, CodexAuthCard));
      return async () => {
        await disposeRemote();
      };
