...
function listenCallback(handler, port) {
...
export class CodexOAuthController {
   ...
   * @param {import('./models.js').CodexModelRegistry} [options.registry] - live model discovery owner; serves the card's model status.
   * @param {() => void} [options.onCredentialChange] - fired after a login or a source switch; the route re-fetches its model manifest.
   * @param {(input: any, init?: any) => Promise<any>} [options.fetchImpl]
   * @param {string} [options.dshAuthPath]
   * @param {string} [options.codexAuthPath]
   * @param {number} [options.callbackPort] - local OAuth callback port; tests use an ephemeral port.
   */
  constructor(options) {
  ...
  source() {
    ...
    return { storage: 'codex', label: 'Shared Codex CLI credentials', path: this.codexAuthPath, writable: false };
  }
  ...
  /** Secret-free model-discovery state for the Settings card. */
  async modelsStatus() {
  ...
  /** Re-fetch the backend's model manifest now; never throws to the card. */
  async refreshModels() {
    if (this.registry === undefined) return { count: 0, source: 'catalog', fetchedAt: null, error: null, changed: false };
  ...
  async selectStorage(storage) {
    ...
    this.lastError = undefined;
    // A different source can mean a different ChatGPT account, so the model
    // manifest the old account answered no longer describes this route.
  ...
  async beginBrowserLogin() {
    ...
    // The one authorization URL for this attempt. It also rides on the
    // pending status: closing the browser tab loses the only other copy, and
    // the card must keep an "open the page again" escape hatch.
    const authorizationUrl = createAuthorizationUrl({ state, challenge });
    let server;
    try {
      ...
      if (code === 'EADDRINUSE') throw new Error(`The OAuth callback port ${this.callbackPort} is already in use. Use Device code login below, or finish the other Codex login first.`);
    ...
    this.pending = {
      kind: 'browser',
      expiresAt,
      view: () => ({ kind: 'browser', expiresAt, url: authorizationUrl }),
      close: async () => {
        clearTimeout(timeout);
  ...
  async finishAuthorizationCode(code, verifier, redirectUri) {
    ...
    this.lastError = undefined;
    // A new credential means a new account answer for the models manifest;
    // discovery runs in the background and adopts what it learns.
    this.onCredentialChange?.();
  }
  ...
  /**
   * User-facing cancel of the interactive login in progress. Closing the
   * browser mid-login must never be a dead end: this releases the callback
   * server (or the device poll loop) immediately, so a new login can start
   * without waiting out the timeout or restarting the host. Safe when idle.
   */
  async cancelLogin() {
    if (this.pending !== undefined) await this.finishPending();
    return this.status();
  ...
  requireWritableSource() {
    ...
    if (source.storage === 'custom') throw new Error('This composition pins a custom authPath. Remove it to let DSH run an interactive login.');
    throw new Error('Shared Codex CLI credentials are read-only. Select DSH-managed credentials before starting an interactive login.');
  }
