 ...
 * @param {string} path - the file it came from.
 * @returns {CodexCredential}
 */
export function parseAuthDocument(parsed, path) {
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new LlmError(`${PKG}: credential file is not a Codex auth document (expected a JSON object); sign in with the Codex CLI (\`codex login\`)`, INVALID_CREDENTIAL);
  }
  ...
  if (doc.tokens !== undefined) {
    ...
    if (tokens === null || typeof tokens !== 'object') {
      throw new LlmError(`${PKG}: credential file has a non-object "tokens" field; sign in again with \`codex login\``, INVALID_CREDENTIAL);
    }
    ...
    if (typeof t.access_token !== 'string' || typeof t.refresh_token !== 'string') {
      if (typeof doc.OPENAI_API_KEY === 'string' && doc.OPENAI_API_KEY.length > 0) {
        throw new LlmError(`${PKG}: credential file holds an API-key login (auth_mode "${String(doc.auth_mode ?? 'apikey')}"), not the ChatGPT subscription this route serves — run \`codex login\` and choose "Sign in with ChatGPT", or use an api-key provider route for that key`, INVALID_CREDENTIAL);
      }
      throw new LlmError(`${PKG}: credential file has no ChatGPT subscription tokens; sign in with the Codex CLI (\`codex login\`) and choose "Sign in with ChatGPT"`, MISSING_CREDENTIAL);
    }
  ...
  throw new LlmError(`${PKG}: credential file is neither a Codex CLI auth file (no "tokens") nor a pi OAuth entry (no "access"/"refresh"); sign in with the Codex CLI (\`codex login\`)`, MISSING_CREDENTIAL);
}
...
export class CodexAuthFile {
  ...
  async ensureLoaded() {
    ...
    try {
      ...
      throw new LlmError(`${PKG}: credential file is not valid JSON (${error instanceof Error ? error.message : String(error)}); sign in again with \`codex login\``, INVALID_CREDENTIAL, { cause: error });
    }
  ...
  async mtime() {
    try {
      ...
      if (/** @type {{code?: string}} */ (error)?.code === 'ENOENT') {
        throw new LlmError(`${PKG}: no Codex credentials in the credential file; sign in with the Codex CLI (\`codex login\`) — or set CODEX_HOME / the route's authPath to the file holding your ChatGPT subscription login`, MISSING_CREDENTIAL);
      }
      throw error;
    }
  }
  ...
  async persist(credential, pair) {
    ...
    await writeAuthDocument(this.path, document);
    const mtimeMs = await this.mtime();
...
/**
 * Build a Codex-CLI-compatible document from a just-completed OAuth exchange.
 * Keeping this wire shape means DSH-managed credentials can become shared
 * credentials simply by selecting the Codex CLI storage mode later.
 * @param {{access: string, refresh: string, idToken?: string, accountId?: string}} tokens
 * @param {Date} [now]
 * @returns {Record<string, unknown>}
 */
export function codexAuthDocument(tokens, now = new Date()) {
  const accountId = tokens.accountId ?? accountIdOf(tokens.access);
  return {
    auth_mode: 'chatgpt',
    OPENAI_API_KEY: null,
    tokens: {
...
/**
 * Atomically persist a newly completed OAuth login with owner-only permissions.
 * The DSH-managed credential parent may not exist on a first login; the Codex
 * CLI parent already exists, but creating it is harmless and makes custom
 * paths work too.
 * @param {string} path
 * @param {Record<string, unknown>} document
 */
export async function writeAuthDocument(path, document) {
  const parent = dirname(path);
  await mkdir(parent, { recursive: true, mode: 0o700 });
  const temp = join(parent, `.auth.json.${process.pid}.${Math.random().toString(36).slice(2)}.tmp`);
  await writeFile(temp, JSON.stringify(document, null, 2) + '\n', { mode: 0o600 });
  try {
    await rename(temp, path);
  } catch (error) {
    // A failed rename must not leave a second copy of the secret behind.
